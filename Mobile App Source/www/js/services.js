	...
	return function(){
		...
		return {
			...
			getEmail: function(email){
				item = item.orderByChild('email').equalTo(email);
				return $firebaseObject(item);
			},
	...
	return function(id){
		...
		return {
			...
			add: function(list,count){
				...
				item.child('user').once('value').then(function(snapshot){
					item.child('countUser').set(snapshot.numChildren());
				});
			},
			leave: function(userid){
				...
				numUser.$loaded(function(){
					...
					item.child('countUser').set((numUser.$value)-1, onSuccess);
				});
			},
...
.factory('Location', function($firebaseObject, $ionicPopup, $ionicPlatform){
	return function(id){
