...
.controller('areacodeCtrl', function($scope, $ionicModal, $http){
 	$http.get('js/areacode.json').then(function(response){
		$scope.areacodes = response.data;
	});
...
.controller('signCtrl', function($scope, $state, $http, $ionicPopup, Login, User, Camera, $filter, $localStorage, $location){
	$scope.data = {};
	$scope.choseArea = {name:"United States","areacode":"1"};
	$scope.showValue = {"type":"password","text":"Show"};
	$scope.login = function(){
	if(!angular.isDefined($scope.data.phone) || !$scope.data.phone){
					 ...
					 { text:'Change'},
					 {
						...
						onTap: function(e){
							...
							$http.head($scope.hostMail+'?action=forgot&phone='+$scope.data.fullPhone).then(function(){
								$scope.data.notification = "Lost Password message has been sent to your email account used to register";
								$scope.hideLoading();
							});
						}
					 }
...
.controller('homeCtrl', function($scope, $localStorage, Notification, $stateParams, $ionicTabsDelegate){
	...
	if(angular.isDefined($stateParams.tab)){
		...
		setTimeout(function(){ $ionicTabsDelegate.$getByHandle("tab-home").select($scope.tab); })
	}
})
.controller('messagesCtrl', function($scope, $ionicPopup, $rootScope, Messages, User, $state, $localStorage) {
	...
	$scope.messages.$loaded(function(){
		...
		$scope.$watch('messages.length', function(newVal, oldVal){
			if(oldVal != newVal) $scope.loadMessage();
		});
	});
...
.controller('messagesDetail', function($scope, $ionicScrollDelegate, $ionicTabsDelegate, $ionicSideMenuDelegate, $timeout, $ionicPopup, IonicClosePopupService, Notification, Block, Messages, Camera, DetailMessages, User, $localStorage, $state, $stateParams, Location){
	...
	$scope.Unread = function(){
		var unread = Messages($localStorage.userLogin.id).getUnread($stateParams.id);
		unread.$loaded(function(){
			if(unread.$value > 0){
				Notification($localStorage.userLogin.id).update(unread.$value);
	...
	$scope.Detail.$loaded(function(){
		$scope.$watch('Detail.length', function(){
			$scope.Unread();
		});
		$scope.$watch('contentBottom', function(){ $scope.toBottom(); });
	});
	$scope.Me = {};
	...
	$scope.Block.$loaded(function(){
		if(!$scope.Block.$value) {
			$scope.contentBottom = 100;
			$scope.messageInput = function(option){
				if(option == "text") $scope.contentBottom = 100;
				else $scope.contentBottom = 220;
			};
...
.controller('contactsCtrl', function($scope, $ionicPopup, IonicClosePopupService, Block, Contacts, ContactsRecommended, User, $localStorage, $filter) {
	...
	$scope.contacts.$loaded(function(){
		...
		$scope.hideLoading();
		$scope.$watch('contacts.length', function(newVal, oldVal){
			if(oldVal != newVal) { $scope.loadContacts(); }
		});
	});
	$scope.showMenu = function(name,id){
	var confirmPopup = $ionicPopup.confirm({
		title: name,
	...
	if($scope.phoneFull.length < 9) { $scope.warning = true }
		...
		$scope.person.$loaded(function(){
			if(angular.isDefined($scope.person.id) && $scope.person.id != $localStorage.userLogin.id){
				$state.go('searchContacts', {id:$scope.person.id});
...
.controller('groupCtrl', function($scope, $http, $state, IonicClosePopupService, UserGroups, Groups, User, $ionicPopup, $localStorage){
	...
	$scope.groups.$loaded(function(){
		...
		$scope.$watch('groups.length', function(newVal, oldVal){
			if(oldVal != newVal) $scope.loadGroup();
		});
		$scope.hideLoading();
	});
	$scope.showPopupMenuGroup = function(id,nameGroup,title) {
		if(nameGroup && angular.isString(nameGroup)) var name = nameGroup;
			...
			var name = '';
			for(var i=0; i<=2; i++){
				if(title[i] && title[i].$value) name += title[i].$value + ', ';
			}
		}
		...
		buttons: [
		  ...
		  { text: 'Add user',type: 'button-clear',onTap: function(e){$state.go('addGroup',{id:id});} },
		  { text: 'View user',type: 'button-clear',onTap: function(e){$state.go('viewGroup',{id:id});} },
		]
		});
	...
	$scope.changeName = function(id,name){
		...
		var myPopup = $ionicPopup.show({
			template: '<input type="text" ng-model="data.name" placeholder="Name" class="light-bg">',
			cssClass: 'popup-confirm-leave',
			scope: $scope,
			buttons: [
			  {
